...
output "dns_name" {
  ...
  value       = element(concat(aws_efs_file_system.default.*.dns_name, [""]), 0)
}

output "id" {
  description = "ID of EFS"
  value       = element(concat(aws_efs_file_system.default.*.id, [""]), 0)
}

output "kms_key_id" {
  description = ""
  value       = element(concat(aws_efs_file_system.default.*.kms_key_id, [""]), 0)
}
...
output "host" {
  ...
  value       = element(concat([module.dns.hostname], [""]), 0)
}
